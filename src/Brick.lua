...
function Brick:hit(key)
    ...
    if not (self.locked == 2) then
        ...
        if self.tier > 0 then
            ...
            if self.color == 1 then
                ...
                if math.random(10) == 10 then
                    table.insert(gStateMachine.current.powerups, Powerup(self.x + 8, self.y, 1))
                end
                if (self.locked == 1 and math.random(1, 5) == 5 and not (gStateMachine.current.paddle.key)) or gStateMachine.current.nextKeyDrop then
                    table.insert(gStateMachine.current.powerups, Powerup(self.x + 8, self.y, 2))
                end
            else
                self.color = self.color - 1
            end
        ...
        if gStateMachine.current.paddle.key then
            self.locked = 0
            gStateMachine.current.score = gStateMachine.current.score + 500
        end
    end
