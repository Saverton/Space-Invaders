...
function LevelMaker:createMap(level)
    ...
    --level is default set as key level, but may lose status later
    local keylevel = math.random(1,3) == 3 and true or false
    if keylevel then
        local lockedRow = false
    end
    ...
    for y = 1, numRows do
        ...
        --whether we want to make a row with locked bricks (0 = normal bricks, 1 = normal bricks, but can drop keys, 2 = locked brick)
        local lockedPattern = 0
        if keylevel then
            if not lockedRow then
                lockedPattern = math.random(1, 2)
            else
                lockedPattern = 1
            end
            if lockedPattern == 2 then
                lockedRow = true
            end
        end
    ...
    --if it was a locked brick level, count the amount of locked bricks in the level. If there are none or it is all bricks,
    --recreate the level.
    if keylevel then
        local lockedBricks = 0
        for k, brick in pairs(bricks) do
            if (brick.locked == 2) then
                lockedBricks = lockedBricks + 1
            end
        end
        if (lockedBricks == 0) then
            for k, brick in pairs(bricks) do
                brick.locked = 0
            end
        elseif lockedBricks == #bricks then
